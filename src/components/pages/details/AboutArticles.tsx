...
import scss from "./AboutArticles.module.scss";
import { useGetArticlesDetailsQuery } from "@/redux/api/skillz";
import { useParams } from "next/navigation";
import Image from "next/image";

const AboutArticles: FC = () => {
  const { id } = useParams();
  const coursesId = Number(id);

  const { data: course } = useGetArticlesDetailsQuery(coursesId);
  if (!course) {
    return (
